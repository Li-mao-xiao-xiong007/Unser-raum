...
ALTER TABLE memories
  ADD CONSTRAINT memories_tone_check CHECK (tone IN ('warm', 'playful', 'neutral', 'cold'));

COMMENT ON COLUMN memories.tone IS '情感温度: warm=暖(优先浮现), playful=轻快(正常浮现), neutral=中性(仅相关性), cold=冷(默认不浮现)';
